...
public class Rotation_Vector extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_rotation__vector);

        firebaseAuth = FirebaseAuth.getInstance();
        firebaseUser = firebaseAuth.getCurrentUser();
        mDatabase = FirebaseDatabase.getInstance().getReference().child("users")
                .child(firebaseUser.getUid()).child("Rotation Vector");


        sm = (SensorManager)getSystemService(SENSOR_SERVICE);
        sensor = sm.getDefaultSensor(Sensor.TYPE_ROTATION_VECTOR);
        textView = (TextView) findViewById(R.id.rotationView);
...
//        Toast.makeText(this, String.valueOf(event.values[0]), Toast.LENGTH_LONG).show();
        ...
        textView.setText("Rotation Vector Orientation\nX: " + String.valueOf(event.values[0]) +
