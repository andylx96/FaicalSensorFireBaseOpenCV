...
public class Light extends AppCompatActivity implements SensorEventListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mDatabase = FirebaseDatabase.getInstance().getReference().child("users")
                .child(firebaseUser.getUid()).child("Light");


        sm = (SensorManager)getSystemService(SENSOR_SERVICE);
        sensor = sm.getDefaultSensor(Sensor.TYPE_LIGHT);
        textView = (TextView) findViewById(R.id.lightView);
...
//        Toast.makeText(this, String.valueOf(event.values[0]), Toast.LENGTH_LONG).show();
        ...
        textView.setText("Ambient Light in Si Lux unit " + String.valueOf(event.values[0]));
